...
dout = sys.argv[2]  # "/projects/p30791/methylation/plots"

meta = pd.read_csv("/projects/p30791/methylation/data/meta.csv")
tissue_type = {idat : region for idat, region in zip(meta["IDAT"], meta["Sample Region"])}

with open(f"{din}/exclude_IDATs.txt", "r") as f:
...
for qc_metric in qc_metrics.keys():
    ax = axs[qc_metrics[qc_metric]["pos"][0], qc_metrics[qc_metric]["pos"][1]]
    sns.violinplot(
        data = qc_all, x = "Tissue type", y = qc_metric, hue = "Status", split = True,
        order = ["Normal", "CUB", "OQ", "AN", "TU"], inner = "quartile", palette = "Set2", ax = ax
    )
    ax.set_title(qc_metrics[qc_metric]["name"])
    ax.set_xlabel("")
